...
function listTables(tables) {
    ...
    inquirer.prompt([{
        ...
        _.each(choices.tables, function(table) {
            ...
            promises.push(columns.get(table).then(listColumns));
            promises.push(relations.get(table).then(listRelations));
        });
...
function listRelations(relations) {
    var table = new Table({
        head: ['Foreign Key', 'Referenced Table', 'Referenced Column']
    });
    winston.info('relations:');
